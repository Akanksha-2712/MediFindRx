...
const HospitalDashboard = () => {
    ...
    const [activeTab, setActiveTab] = useState('bed'); // 'bed', 'appointments', 'ambulance'
    ...
    const fetchHospitalData = async () => {
        try {
            ...
            // Get Hospital Details (Robust fetch)
            let { data: hospitalList, error: hospitalError } = await supabase
                .from('hospitals')
                .select('*')
                .eq('id', user.id)
                .limit(1);

            let hospitalData = hospitalList?.[0];

            // Auto-Heal: If missing, create it
            if (!hospitalData && !hospitalError) {
                console.log("Hospital record missing, attempting auto-creation...");
                const { data: newHospitalList, error: createError } = await supabase
                    .from('hospitals')
                    .insert([{
                        id: user.id,
                ...
                hospitalData = newHospitalList?.[0];
            }
        ...
        <div className="max-w-4xl mx-auto p-8 space-y-12 pb-24">
            ...
            <div className="flex flex-wrap gap-4 border-b border-gray-100 pb-1">
                <button
                    ...
                    className={`flex items-center gap-2 px-6 py-3 rounded-t-xl font-bold transition-all ${activeTab === 'bed'
                ...
                >
                    <Bed className="h-5 w-5" /> Bed Management
                </button>
                <button
                    onClick={() => setActiveTab('appointments')}
                    className={`flex items-center gap-2 px-6 py-3 rounded-t-xl font-bold transition-all ${activeTab === 'appointments'
                            ? 'bg-white text-secondary border-b-2 border-secondary shadow-sm'
                            : 'bg-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50'
                        }`}
                >
                ...
                <button
                    ...
                    className={`flex items-center gap-2 px-6 py-3 rounded-t-xl font-bold transition-all ${activeTab === 'ambulance'
                            ? 'bg-white text-red-600 border-b-2 border-red-600 shadow-sm'
                            : 'bg-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50'
                        }`}
                >
                    <Ambulance className={`h-5 w-5 ${emergencyRequests.length > 0 ? 'animate-pulse' : ''}`} />
                    Ambulance SOS
            ...
            {/* Bed Management */}
            {activeTab === 'bed' && (
                <section className="bg-white rounded-b-xl rounded-tr-xl shadow-sm border p-6 transition-all hover:shadow-md animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            {activeTab === 'appointments' && (
                <section className="bg-white rounded-b-xl rounded-tr-xl shadow-sm border p-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                    ...
                    <div className="mt-6 space-y-3">
                        ...
                        {appointments.map(appt => (
                            <div key={appt.id} className={`flex justify-between items-center bg-gray-50 p-3 rounded-lg border border-gray-100 ${appt.status === 'completed' ? 'opacity-60' : ''}`}>
                                <div>
                                    ...
                                    <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full ${appt.status === 'completed' ? 'bg-green-100 text-green-700' :
            ...
            {/* Emergency SOS Feed */}
            {activeTab === 'ambulance' && (
                <section className="bg-white rounded-b-xl rounded-tr-xl shadow-sm border p-6 border-red-50 animate-in fade-in slide-in-from-bottom-4 duration-300">
